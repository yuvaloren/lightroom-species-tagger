...
for f in Installing Using-it Settings; do
	[ -s "$W/$f.md" ] || die "extracted nothing for $f — did a README heading change? update scripts/sync-wiki.sh"
done

# ---- pages copied from docs/ ------------------------------------------------------
cp docs/TROUBLESHOOTING.md "$W/Troubleshooting.md"
cp docs/FAQ.md "$W/FAQ.md"
cp docs/PRIVACY.md "$W/Privacy.md"
