...
# domain.
#
#   bash scripts/build-pkg.sh                # after sign-macos.sh has run
#   ALLOW_UNSIGNED=1 bash scripts/build-pkg.sh   # unsigned pkg (never distributable)
#
...
# MACOS_INSTALLER_SIGN_IDENTITY in scripts/signing.env, or let this script
...
[ -f "$ZIP_MAC" ] || die "no $ZIP_MAC — run scripts/sign-macos.sh first (the pkg payload comes from the signed mac zip)"
...
RES="$TMP/resources"
mkdir -p "$RES"
cp installer/mac/resources/welcome.html installer/mac/resources/conclusion.html "$RES/"
...
if [ -n "$IDENTITY" ]; then
	...
	if [ -n "${NOTARY_KEY_P8_BASE64:-}" ]; then
		...
		die "no notary credentials — set NOTARY_PROFILE or NOTARY_KEY_* (see docs/SIGNING.md)"
	fi
