...
#
#   just exchange        # or: bash build/build-exchange.sh [X.Y.Z]
...
#   a CA cert is optional). Generated once into build/exchange-cert.p12 with
#   a random password in build/exchange-cert.pass (both gitignored) and
#   reused so the cert stays stable across versions. Override with
#   ZXP_CERT_P12 / ZXP_CERT_PASSWORD (e.g. in build/signing.env).
...
CERT="${ZXP_CERT_P12:-build/exchange-cert.p12}"
PASS="${ZXP_CERT_PASSWORD:-}"
if [ -z "$PASS" ] && [ -f build/exchange-cert.pass ]; then
	PASS="$(cat build/exchange-cert.pass)"
fi
if [ ! -f "$CERT" ]; then
	say "generating a self-signed signing cert ($CERT)"
	if [ -z "$PASS" ]; then
		PASS="$(openssl rand -hex 24)"
		printf '%s' "$PASS" > build/exchange-cert.pass
		chmod 600 build/exchange-cert.pass
	fi
	"$ZXPSIGN_CACHE" -selfSignedCert US CA "Yuval Oren" "Yuval Oren" "$PASS" "$CERT" >/dev/null \
		|| die "self-signed cert generation failed"
fi
[ -n "$PASS" ] || die "cert exists but no password — set ZXP_CERT_PASSWORD or restore build/exchange-cert.pass"
