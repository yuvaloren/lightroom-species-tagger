...
# build/build.lua (dev/CI zips) and build/sign-macos.sh (signed release zips),
# so the two paths can never drift.
#
#   bash build/package-zips.sh <version-label>
#
...
[ -d "$BUNDLE" ] || die "no bundle at $BUNDLE — compose first (lua build/build.lua --no-zip)"
