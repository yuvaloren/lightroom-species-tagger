#!/usr/bin/env bash
# check-go-eol.sh — fail when the Go toolchain pinned in src/helper/go.mod (the
...
PIN="$(grep -E '^(toolchain go|go )' src/helper/go.mod | tail -1 | grep -oE '[0-9]+\.[0-9]+(\.[0-9]+)?')"
[ -n "$PIN" ] || { echo "::error::could not read the Go pin from src/helper/go.mod"; exit 1; }
...
if cycle.get('eol') is not False:
    print(f"::error::pinned Go {pinned} is END-OF-LIFE — bump src/helper/go.mod (go + toolchain) to the newest stable Go")
    sys.exit(1)

newest = str(cycles[0].get('cycle'))
if newest != cycle_wanted:
    print(f"::warning::Go {cycle_wanted} is no longer the newest cycle ({newest} is out) — one more Go release makes it EOL; bump src/helper/go.mod soon")
