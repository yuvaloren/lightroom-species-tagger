...
#
#   bash build/build-win-installer.sh          # after the -win zip exists
#
...
#
# Signing: delegated to build/sign-win.sh (Azure Trusted Signing — currently
...
[ -f "$ZIP_WIN" ] || die "no $ZIP_WIN — package first (build/package-zips.sh runs inside sign-macos.sh / build.lua)"
...
makensis -V2 \
	...
	-DPAYLOAD="$PAYLOAD" \
	-DOUTFILE="$PWD/$OUT" \
	build/installer/win/SpeciesTagger.nsi

[ -f "$OUT" ] || die "makensis produced no $OUT"

bash build/sign-win.sh "$OUT"
say "built $OUT"
