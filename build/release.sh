...
#      the gitignored build/signing.env this script sources.
...
set -euo pipefail
cd "$(dirname "$0")/.." # build/release.sh -> repo root
...
if [ -f build/signing.env ]; then
	say "loading build/signing.env"
	set -a
...
if [ "$NO_PUBLISH" = "0" ]; then
	...
	[ -x .lua-env/bin/lua ] || { say "bootstrapping the pinned Lua toolchain (build/setup.sh)"; bash build/setup.sh; }
	...
	say "lint"
	luacheck -q src test build
	say "tests (Lua specs + Go helper units)"
	busted
...
if [ ! -x .lua-env/bin/lua ]; then
	say "bootstrapping the pinned Lua toolchain (build/setup.sh)"
	bash build/setup.sh
fi
export PATH="$PWD/.lua-env/bin:$PATH"

# 2. Compose the release bundle (darwin-universal + win-x64 + win-arm64).
# build.lua cross-compiles the Go helper itself (all four targets + the
# universal mac binary — the exact bytes that ship), then composes; --no-zip
# because sign-macos.sh packages the signed zips in the next stage.
say "composing the release bundle (cross-compiles the lens helper)"
lua build/build.lua --no-zip

# 4. Developer ID sign + notarize + package the three zips.
say "signing + packaging"
bash build/sign-macos.sh ${SIGN_ARGS[@]+"${SIGN_ARGS[@]}"}
...
ALLOW_UNSIGNED="$ALLOW_UNSIGNED" bash build/build-pkg.sh
