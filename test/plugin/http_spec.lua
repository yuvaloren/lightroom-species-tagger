...
describe( 'Http._test.interpretHashResult (helper stdout -> hash() contract)', function()
	local Z = '0000000000000000'
	it( 'rebuilds the array index-by-index, false where the helper answered null', function()
		-- dkjson turns JSON null into nil (a hole); the rebuild keeps it addressable
		local hs = T.interpretHashResult( { ok = true, hashes = { [ 1 ] = Z, [ 3 ] = Z } }, nil, 3 )
		assert.same( { Z, false, Z }, hs )
	end )
	it( 'returns all-false when the helper omitted an empty hashes array', function()
		assert.same( { false, false }, T.interpretHashResult( { ok = true }, nil, 2 ) )
	end )
	it( 'passes a runHelper failure straight through', function()
		local hs, err = T.interpretHashResult( nil, 'helper exploded', 2 )
		assert.is_nil( hs )
		assert.equal( 'helper exploded', err )
	end )
	it( 'surfaces the helper error on ok = false', function()
		local hs, err = T.interpretHashResult( { ok = false, error = 'could not read hash list' }, nil, 1 )
		assert.is_nil( hs )
		assert.matches( 'could not read hash list', err )
	end )
	it( 'rejects non-table output', function()
		local hs, err = T.interpretHashResult( 42, nil, 1 )
		assert.is_nil( hs )
		assert.matches( 'unusable output', err )
	end )
	it( 'coerces a non-string entry to false rather than passing junk to Burst', function()
		assert.same( { false }, T.interpretHashResult( { ok = true, hashes = { 12345 } }, nil, 1 ) )
	end )
end )
