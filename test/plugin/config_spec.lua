...
describe( 'Config burst settings', function()
	it( 'defaults burst detection ON with a 1-second gap', function()
		local c = Config.load( nil )
		assert.is_true( c.burstDetect )
		assert.equal( 1, c.burstGapSeconds )
	end )
	it( 'preserves a stored false burstDetect', function()
		assert.is_false( Config.load( { burstDetect = false } ).burstDetect )
	end )
	it( 'coerces and clamps the gap (free-typed field)', function()
		assert.equal( 3, Config.load( { burstGapSeconds = '3' } ).burstGapSeconds )
		assert.equal( 1, Config.load( { burstGapSeconds = 'abc' } ).burstGapSeconds )
		assert.equal( 1, Config.load( { burstGapSeconds = 0 } ).burstGapSeconds )
		assert.equal( 10, Config.load( { burstGapSeconds = 99 } ).burstGapSeconds )
	end )
end )
