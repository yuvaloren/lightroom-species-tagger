...
describe( 'Keywords.plan', function()
	...
	it( 'both mode = hierarchy + the two flat keywords', function()
		local p = Keywords.plan( OCTOPUS, { mode = 'both' } )
		assert.equal( 'Animalia', p.nodes[ 1 ].path[ 1 ] ) -- hierarchy node, top-level kingdom
		local a = attachSet( p )
		assert.is_true( a[ 'Day octopus' ] )
		assert.is_true( a[ 'Octopus cyanea' ] )
